...

{
    ...
    oFile    << "#"   << day.append( header.Date.begin () + 3, header.Date.begin () + 5 ) 
             << " "   << Month << " "    << year.append( header.Date.begin () + 6, header.Date.end () );
    ...
    while ( ( iFile >> Entry_Temp.Network_Code ) && ( Numb_of_valid_entries < 300)) 
    {
       ...
       // Checking for Orientation
        ...
        if ( ( Entry_Temp.Orient.length() < 1 ) || ( Entry_Temp.Orient.length() > 3)) 
        {
            flag = 5;
        }
        else 
...
// This "Mag_size_check" function checks the validity of Mag size as a positive real number (>0).
